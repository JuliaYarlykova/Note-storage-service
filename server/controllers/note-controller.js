...
const postNote = async (request, response) => {
	try {
		const { title, titleFolder, text, colorText } = request.body
...
const patchNote = (request, response) => {
	console.log(request.body)
	const { titleold, title, idFolder } = request.body
	const idF = Folder.findOne({ where: { title: idFolder } })
	Note.findOne({ where: { title: titleold } }).then(note => {
		if (!note) {
			return response.status(404).send('конспект не найден')
		}
		note
			.update({
				title: title,
