<template>
	<main class="page-folder">
		<div class="page-folder__container">
			...
			<div class="page-folder__wrap">
				<div class="page-header__for" v-for="obj in objs">
					<CardFolder
						@editBtn="edit"
						@clickFolder="toFolder(obj.title)"
		...
		<EditFolderPP v-show="activeEdit.act" @deleteF="close" />
	</main>
</template>
...
export default {
	...
	methods: {
			...
			if (act) {
				axios
					...
					.then(res => {
						this.objs.forEach(
							(d, index) =>
								d.title === this.active.title && this.objs.splice(index, 1)
						)
					})
		...
		text_input() {
			...
			axios.post(`http://localhost:7335/api/folder`, this.req).then(res => {
